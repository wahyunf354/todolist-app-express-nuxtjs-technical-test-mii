...
const handleSubmitTodo = async () => {
  await $fetch(`${config.public.apiBase}/api/auth/singup`, {
    method: "POST",
    body: {
      id: username.value,
      name: name.value,
    ...
    <form @submit.prevent="handleSubmitTodo()" class="py-5">
      ...
      <div class="mb-3">
        <input
          ...
          name="password"
          type="password"
          class="mt-1 w-full border-gray-200 align-top shadow-sm sm:text-md p-3 text-slate-700"
      ...
      <div class="flex gap-2">
        ...
        <AtomsButtonLink label="Login" to="/auth/login" bgColor="bg-gray-300" />
      </div>
    </form>
  </div>
</template>
